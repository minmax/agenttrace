...
`agenttrace` compresses coding-agent session logs (Claude Code, Codex CLI, pi, Cursor, Qwen Code, Kimi, MiniMax Code) into token-efficient Markdown traces with lossless `@L<line>` references back into the original logs.
...
```bash
npm install
npm run agenttrace -- <command>   # CLI via tsx, no build step (Node ≥ 22.19)
npm run agenttrace -- work --help # work-server protocol reference
...
Refine env: `AGENTTRACE_MODEL`, `AGENTTRACE_FAST_MODEL`, `AGENTTRACE_BASE_URL`, `AGENTTRACE_API_KEY` — any OpenAI-compatible gateway.
