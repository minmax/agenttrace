#!/usr/bin/env node
/**
 * agenttrace — compress coding-agent session logs (claude, codex, pi, qwen,
 * kimi, minimax; cursor via the work server) into token-efficient,
 * losslessly-referenced MD traces.
 *
 *   agenttrace distill [rootDir]  — deterministic base trace (fast, no LLM)
 *   agenttrace refine  [rootDir]  — full pipeline: streaming FAST pass + SMART pass + bank
 */
...
const COMMON_OPTIONS = {
	...
	only: { type: "string" },
	max: { type: "string" },
	model: { type: "string", default: process.env["AGENTTRACE_MODEL"] ?? "anthropic/claude-sonnet-4-5" },
	"fast-model": { type: "string", default: process.env["AGENTTRACE_FAST_MODEL"] },
	"base-url": { type: "string", default: process.env["AGENTTRACE_BASE_URL"] },
	"api-key": { type: "string", default: process.env["AGENTTRACE_API_KEY"] ?? "stub" },
} as const;
...
function printUsage(): void {
	console.log(`agenttrace — сжатые трейсы агентских сессий

Использование:
  agenttrace distill [rootDir] [options]     детерминированный трейс-скелет (без LLM)
  ...
  agenttrace refine [rootDir] [options]      полный: FAST-проход + SMART + банк
    --fast-model <p/id>  простая модель ($AGENTTRACE_FAST_MODEL; по умолчанию = --model)
    --model <p/id>       умная модель ($AGENTTRACE_MODEL; anthropic/claude-sonnet-4-5)
    --base-url <url>     OpenAI-совместимый эндпоинт ($AGENTTRACE_BASE_URL)
    --api-key <key>      ключ ($AGENTTRACE_API_KEY, по умолчанию stub)
