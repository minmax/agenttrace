...
function printUsage(): void {
	console.log(`agenttrace — compressed traces of coding-agent sessions

Usage:
  agenttrace distill [rootDir] [options]     deterministic trace skeleton (no LLM)
  ...
  agenttrace refine [rootDir] [options]      full: FAST pass + SMART + bank
    --fast-model <p/id>  fast model ($AGENTTRACE_FAST_MODEL; defaults to --model)
    --model <p/id>       smart model ($AGENTTRACE_MODEL; anthropic/claude-sonnet-4-5)
    --base-url <url>     OpenAI-compatible endpoint ($AGENTTRACE_BASE_URL)
    --api-key <key>      API key ($AGENTTRACE_API_KEY, default stub)
