...
function printWorkUsage(): void {
	console.error(`agenttrace work — agent mode: the script hands out data in layers, the agent compresses it and submits the result

  agenttrace work init --state <dir>     build state: cursor export, session registry, det skeletons
  ...
  agenttrace work release --state <dir> <jobId>           give up the batch
  agenttrace work layer --state <dir> pass1 [--cursor <tok>] [--limit <n>]
      paged overview of the layer's batches
  agenttrace work finalize --state <dir>                  traces per project + bank + metrics
