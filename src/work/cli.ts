/**
 * Agent-facing CLI for the work server (`agenttrace work ...`).
 *
 ...
 *
 *   agenttrace work init     --state <dir> [roots/cursor flags]   build state (resumable)
 *   agenttrace work status   --state <dir>                        counters per layer/project
 *   agenttrace work claim    --state <dir> --layer <L> --worker <name>
 *   agenttrace work submit   --state <dir> <jobId>                JSON body on stdin
 *   agenttrace work release  --state <dir> <jobId>
 *   agenttrace work layer    --state <dir> <L> [--cursor] [--limit]
 *   agenttrace work finalize --state <dir>                        traces + bank + metrics
 */
...
export async function runWorkCli(argv: string[]): Promise<number> {
	...
	const shared = {
		state: { type: "string", default: join(process.cwd(), ".agenttrace-work") },
	} as const;
	...
	try {
		switch (sub) {
			...
			case "submit": {
				...
				if (jobId === undefined) {
					emit({ ok: false, error: "usage: agenttrace work submit <jobId>  (JSON on stdin)" });
					return 1;
				}
			...
			case "release": {
				...
				const jobId = positionals[0];
				if (jobId === undefined) {
					emit({ ok: false, error: "usage: agenttrace work release <jobId>" });
					return 1;
				}
			...
			case "layer": {
				...
				if (layer !== "pass1" && layer !== "pass2") {
					emit({ ok: false, error: "usage: agenttrace work layer <pass1|pass2> [--cursor <tok>] [--limit <n>]" });
					return 1;
				}
...
function printWorkUsage(): void {
	console.error(`agenttrace work — агентский режим: скрипт раздаёт данные слоями, агент сжимает и сдаёт результат

  agenttrace work init --state <dir>     собрать состояние: cursor-экспорт, реестр сессий, det-скелеты
  ...
  agenttrace work status --state <dir>                    счётчики по слоям/проектам
  ...
  agenttrace work submit --state <dir> <jobId> < result.json
      сдать результат (JSON на stdin); ошибки валидации → ok:false, чини и сдавай снова
  agenttrace work release --state <dir> <jobId>           отказаться от пачки
  agenttrace work layer --state <dir> pass1 [--cursor <tok>] [--limit <n>]
      постраничный обзор пачек слоя
  agenttrace work finalize --state <dir>                  трейсы по проектам + банк + метрики
