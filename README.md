...
agenttrace distill ~/projects/myapp --out traces/myapp

# full pipeline: PASS-1 (FAST) + PASS-2 (SMART) + reasoning bank
agenttrace refine ~/projects/myapp \
...
agenttrace distill ~/projects/myapp --out traces/myapp --repo-bound
```

Both commands discover sessions whose `cwd` in the log matches the given directory or any subdirectory. From a repo clone (no global install, no build step) prefix every command with `npm run agenttrace --`.
...
```bash
agenttrace work init     --state ~/agenttrace-run --claude-root ~/.claude/projects --pi-root ~/.pi/agent/sessions
agenttrace work claim    --state ~/agenttrace-run --layer pass1 --worker agent-1
agenttrace work submit   --state ~/agenttrace-run p1-<sid>-w3 < answer.json
agenttrace work release  --state ~/agenttrace-run p1-<sid>-w3
agenttrace work status   --state ~/agenttrace-run
agenttrace work reindex  --state ~/agenttrace-run --window-tokens 20000
agenttrace work finalize --state ~/agenttrace-run   # traces/<project>/*.md (private, with @L) + traces-repo/traces/<project>/*.md (same repo-bound flavor as distill --repo-bound) + bank + metrics
```
...
Any OpenAI-compatible endpoint via `--base-url`. Defaults to `anthropic/claude-sonnet-4-5` (or `$AGENTTRACE_MODEL`). Use `--fast-model` for a cheaper first pass — e.g. `--fast-model my-gateway/minimax/MiniMax-M3 --model my-gateway/zai/glm-5.3-flash --base-url http://localhost:10081/v1`.
...
- [AGENTS.md](AGENTS.md) — for coding agents: setup, source layout, the `agenttrace work` protocol, invariants, and gotchas.
...
npm run agenttrace   # CLI without build (tsx)
```
